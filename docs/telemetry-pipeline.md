...
[Router](README.md) · Read for the measurement record, bounded buffer, on-device Parquet, SD durability and later object-storage upload. Hardware-level SD rules remain in [CoreS3 storage](cores3-storage.md). Implementation snapshot **2026-09-17** (firmware v6 flashed the same day); measured runs belong in [bench-verified](bench-verified.md), separately from planned validation.
...
| `event_time_utc_ns` | Nullable INT64 UTC estimate in nanoseconds, with units explicit in the name. Schema v3 (source 2026-09-17) annotates it and `clock_anchor_utc_ns` with `TIMESTAMP(NANOS, isAdjustedToUTC=true)`: PyArrow reads `timestamp[ns, tz=UTC]`, DuckDB 1.5.5 reads `TIMESTAMP WITH TIME ZONE` at microsecond precision (`epoch_ns()` recovers the integer; firmware values are microsecond-derived). Absent until the host explicitly supplies UTC. |
| `clock_status`, `clock_epoch` | Status 0 = unsynchronized, 1 = host estimate supplied on this boot, 2 = restored at boot from the BM8563 RTC (firmware v6.2, source 2026-09-18). Code 2 is the last host estimate the RTC was written with, at whole-second resolution plus RTC drift since that sync; it is dated (Hive tree, not `unsynced`) but a reader that wants host-fresh time should filter on `clock_status = 1` or on `clock_epoch` after the sync. Epoch increments at each anchor, RTC seed included; files split when it changes. No measured clock-uncertainty bound or synchronized status is claimed. |
...
Use `pixi run parquet-device sync-time --port <port>` to explicitly supply this host's current UTC estimate, or `--sync-time` with `parquet-device bench`. The serial `parquet time <epoch-seconds>` command anchors that value to the device monotonic clock and reports the anchor. This is not NTP; command/transport latency and host error are not measured. Previously buffered rows retain their original timestamps and epoch. Persisting a separate uncertainty-bearing anchor journal is future work.
...
The storage worker creates a uniquely named `.partial` file exclusively when the first RAM batch of a window completes (`<prefix>_<boot>_<first>-<attempt>.partial`). Each completed RAM batch (≤ 90 rows) is written as one row group — one PLAIN page per column plus the chunk's statistics kept in PSRAM — followed by `fflush` and `fsync`, so those rows are on the card before the next batch starts. At the window boundary, after `kMaxRowGroups` (8) groups, or on an explicit flush/codec/interval/reboot command, the worker appends the footer, checks write counts, calls `fflush` and `fsync`, and closes. It then reads the file back to check length, leading/trailing `PAR1` and footer bounds, and computes a whole-file CRC32 before renaming to `<prefix>_<boot>_<first>-<last>-<attempt>.parquet`. With the default 900-second window a file has exactly one row group, as before; 1800/3600 s give two/four. A reset between two row groups leaves a footer-less `.partial` whose earlier groups are complete on the card; it is retained and reported (`part` in the status document counts it), and rebuilding its footer from the page headers is possible host-side but not implemented. This structural check is not a full Parquet decoder: PyArrow and DuckDB perform host conformance validation. Only finalized files are eligible for later upload; never append rows to one. At boot the current implementation lists both suffixes and retains partial files without repair. FAT rename is not assumed crash-atomic, and automatic formatting is disabled.
...
| --- | --- | --- |
| [Apache Parquet format](https://github.com/apache/parquet-format/releases/tag/apache-parquet-format-2.14.0) | 2.14.0 (2026-09-11), Apache-2.0 | The wire specification. Metadata and page headers use Thrift Compact Protocol; it is not an embedded writer library. 2.13 added `Statistics.nan_count` and `IEEE_754_TOTAL_ORDER` with new float-statistics writer rules; 2.14 adds only the `FILE` logical type and `INT96_TIMESTAMP_ORDER`. The writer follows the 2.13 `TYPE_ORDER` rules (checked 2026-09-17). |
...
- one immutable file per rotation window holding one row group per completed 90-row batch (up to eight; one at the default 900 s);
- flat INT32/INT64/FLOAT columns; the logger provides definition levels for every column, always populating identity/status fields and leaving unavailable measurements/time null; the two UTC INT64 fields carry `TIMESTAMP(NANOS, isAdjustedToUTC=true)`;
- PLAIN encoding and Data Page V1, one page per column chunk;
- column-chunk statistics per row group: `null_count` always; `min_value`/`max_value` with `is_min_value_exact`/`is_max_value_exact` when at least one non-NaN value is present; `nan_count` on FLOAT; `FileMetaData.column_orders` = `TYPE_ORDER` for every leaf (mandatory once bounds are written). Floats follow the parquet-format 2.13 `TYPE_ORDER` rules — NaN excluded from bounds, `-0.0` written for a zero minimum and `+0.0` for a zero maximum — because PyArrow 25 and DuckDB 1.5 still ignore statistics under the newer `IEEE_754_TOTAL_ORDER`. The deprecated `Statistics.min`/`max` pair is omitted to keep the 77-chunk footer small; every targeted reader uses the `*_value` fields;
- row-group metadata `file_offset`, `total_compressed_size`, `ordinal` and `sorting_columns` (ascending `sequence`); `ColumnChunk.file_offset` stays 0 as the 2.14 thrift now asks;
- UNCOMPRESSED by default after reboot, or session-selectable LZ4_RAW using pinned LZ4 1.10.0;
- bounded page-codec buffers and correct compressed/original byte counts; no Parquet page CRC32 yet (deferred on 2026-09-17), while serial/BLE readback has a separate whole-file CRC32;
- a fixed 90-row logger batch and bounded streaming buffers; footer records for eight row groups × 96 columns live in the PSRAM workspace (about 50 KB per open file);
- a minimal writer-only Thrift Compact implementation in `parquet_writer.cpp` (streaming `Writer` class; `write_parquet()` wraps one group), without Arrow or Carquet dependencies; `tools/parquet_footer.py` is the matching host-side decoder used by the tests;
- `.partial` -> row group -> sync (repeat) -> footer -> sync -> close -> footer validation -> final rename.

Footer cost, measured on the host fixture with 77 columns × 90 incompressible rows (not real data): 4,484 bytes before statistics, 6,000 with them; 10.4 KB for two groups and 19.3 KB for four. Per-chunk metadata (~57 bytes × 77, mostly the required `path_in_schema` string) dominates, so several row groups per file cut the footer share only from about 17 % to 14 %. Hourly files are worth having for four times fewer files and per-quarter-hour statistics, not for bytes; bytes come from LZ4 and from the phone's derived daily files.

Host conformance checks use PyArrow and DuckDB, including nulls and exact integer values, plus a field-level decode of the footer. Empty, one-row, boundary-value, wide/maximum-row, four-group "hourly" and eight-group fixtures pass the host sanitizer test, including `-0.0`/`+0.0`, all-NaN and all-null bounds; unsynced and dated single-group files pass real SD readback, and the first two-group v6 device file (105 rows across a window boundary) passed both readers and the footer decode on 2026-09-17. Add multiple-page cases only if that feature is introduced. The automatic 60-row run and current Hive-schema checks are recorded separately in [bench-verified](bench-verified.md); longer runs and failure cases remain open. The [compression experiment](compression-benchmark.md) screens Snappy/LZ4/Zstd on the host and adds an identical-row LZ4_RAW comparison on the board. Measure complete-file bytes, CPU time and memory bounds; on-device Snappy/Zstd and longer codec runs remain open. Separately run repeated power cuts during data, page header, footer, sync and rename before claiming recovery guarantees. The cited Zstd wrapper's memory figures are specific to its dictionary workload, not a CoreS3 budget.
