...
The flag scan (`AdvertFlagScan`, two offloaded filters per bonded device, receiver de-duplicating on `boot16:fin:flags`, re-armed by every periodic run) is what fired in the measured run: `ActivityManager: Start proc … for broadcast {…AdvertScanReceiver}` 19 s after `PARQUET READY`, `trigger=ADVERTISEMENT`, LAN sync, done at +28 s.

## Measure before believing

Measured 2026-09-18 on the OnePlus 7 Pro ([bench](bench-verified.md#board-1-advertising-payload-wakes-the-phone-firmware-v63-protocol-21)): (2) the controller accepted two service-data-mask filters and matched one — offloaded filtering works on this phone; (1) flag scan: file close → app process started 19 s → file on the phone 28 s, one event. Still to record, with firmware hash, app commit and phone model: (1) the same latency for CDM alone and its distribution over many events; (3) whether screen-off LAN syncs survive with the CDM exemption active (the socket-kill signature is `InetDiagMessage: Destroyed live tcp sockets for uids=`); (4) whether `setForeground` in the worker stops being refused once associated (it was refused in the measured run, no association yet); (5) phone battery over a day with the scan registered versus without; (6) a Wi-Fi-arrival run end to end. A one-off `EVENT_BLE_APPEARED` on a phone that sat next to the device all day proves the presence path exists, not that files arrive sooner — the flag scan is what changes that, and it did.
