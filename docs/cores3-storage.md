...
UTC is supplied explicitly with `pixi run parquet-device sync-time --port <port>` or the bench helper's `--sync-time`; it is a host estimate, not a validated NTP clock. Until then, files remain under `output/station=<UUID>/unsynced/boot=<boot>/`, with null UTC columns. Since firmware v6.2 that estimate is also written to the BM8563 RTC and restored at the next boot (rows carry `clock_status = 2`), so `unsynced/` only fills on a board whose RTC was never set or lost power (see [telemetry pipeline](telemetry-pipeline.md)). Subsequent clock updates create a new clock epoch and split the batch without changing previous rows. Object-storage upload, cloud compaction and Iceberg are later work; finalized SD files are retained locally.
