...
[Router](README.md) · Read for toolchain, dependency, driver, or runtime changes. Snapshot **2026-09-06**; versions below are upstream releases, not a combination built or tested on this board.
...
- **Budget the SDK download before promising a build.** Measured on 2026-09-06, roughly 1.3 GB landed in the cache in about 20 minutes and neither an Arduino nor an ESP-IDF bootstrap had finished. Observed sizes were arduino-cli about 34 MB, the Arduino esp32 core still incomplete at 228 MB, and ESP-IDF tools at 497 MB alongside a 563 MB shallow clone. Treat a first bootstrap as tens of minutes, run exactly one at a time, and never relocate the cache midway, which restarts transfers already in flight.
